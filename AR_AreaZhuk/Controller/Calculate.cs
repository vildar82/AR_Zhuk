...
namespace AR_AreaZhuk.Controller
{
    ...
    {
        static Dictionary<string, double[]> dictCalcAreas;

        public static void Init()
        {
            dictCalcAreas = new Dictionary<string, double[]>();
        }

        public static double[] GetAreaFlat (int floors, RoomInfo flat, PIK1.C_Flats_PIK1_AreasRow currentFlatAreas)
        {            
            string key = floors + flat.ShortType + "_" + flat.Joint;
            double[] area;
            if (dictCalcAreas.TryGetValue(key, out area))
            {
                return area;
            }
            area = new double[5];//1 - общая площадь, 2 - жилая площадь,3 - площадь этажа/4 - площадь этажа без ЛЛУ/5 - площадь этажа с учетом ЛЛУ
